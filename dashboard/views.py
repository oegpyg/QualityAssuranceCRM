...
def demoOne(request):
    ...
    tasks = core_models.Task.objects.filter(assignedTo=request.user)
    ctx = {'projects': projects, 'release': '', 'tasks': tasks}
    return render(request, 'pages/dashboards/demo_nine.html', context=ctx)
